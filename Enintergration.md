# Express transaction API - No need to redirect URL
...
* <span style = "color: red">API buying & selling: The interface returns the order submitted result in real-time, and one client could only generate one order in the processing period. </span>

* <span style = "color: red">API batch selling: One client is allowed to send multiple selling orders at the same time, orders will be queued for processing. The successful submission of this batch does not mean that all orders can be successfully generated. If a order request is not generated (e.g. caused by the reason of insufficient merchant balance.), there would be no callback for it! </span>
...
|orderType|Order type （1.Express buy 2.Express sell）|Integer||Y| |
...
|payCardBank|Payee’s bank name (required for express selling order). If payCoinSign is “vnd”, please fill in the bank name accurately refer to  <a href="#vnd_area">“The Full List of Bank in VND Area.”</a>|string||N| |
...
|coinAmount|USDT quantity of the order. ( coinAmount and total are alternative parameters, <br>if both are sent, total would take precedence.<br> If you only send CoinAmount, the converted fiat currency amount will be rounded up to the whole digit and displayed on the cashier page if it's originally not an integer.)|BigDecimal||Y| |
...
#### <a name="31">Express batch selling API</a>
...
|total|Fiat currency amount of the order. ( coinAmount and total are alternative parameters, and when both are filled in, total takes precedence)|bigDecimal to string type||N| |
